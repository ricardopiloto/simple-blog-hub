...
Os releases são versionados por tag (ex.: `v1.9`, `v1.10`, `v2.0`, `v2.1`, `v2.2`, `v2.3`, `v2.3.1`, `v2.3.2`, `v2.4`, `v2.4.1`, `v2.5`, `v2.5.1`, `v2.5.2`, `v2.5.3`, `v2.6.0`, `v2.6.1`, `v2.6.2`). O histórico de alterações está em **[CHANGELOG](docs/changelog/CHANGELOG.md)**.
...
- **Contas** (`/area-autor/contas`): qualquer autor edita o próprio perfil (nome, descrição, senha); Admin gere todas as contas (**Nova conta** com e-mail e nome — senha padrão aplicada no servidor; troca obrigatória no primeiro login), editar, resetar senha, excluir. Critério mínimo de senha: 8 caracteres, uma maiúscula, uma minúscula e um número. Secção **Cloudflare Workers AI** (Account ID, API Token, modelo de imagem) para autores que usam Geração de Imagem.
...
- **Atualizar o código (local e Docker):** [**ATUALIZAR-SERVIDOR-DOCKER-CADDY**](docs/deploy/ATUALIZAR-SERVIDOR-DOCKER-CADDY.md) — pull, build e execução em desenvolvimento (API, BFF, frontend) e em produção (rebuild contentores, frontend, cópia para document root); scripts de banco de dados manuais quando aplicável. Quem está na **v1.9** e atualiza para **v1.10**: [**ATUALIZAR-1-9-PARA-1-10**](docs/deploy/ATUALIZAR-1-9-PARA-1-10.md). Quem está na **v2.5.3** (ou **v2.6.0** / **v2.6.1**) e atualiza para **v2.6.2**: [**ATUALIZAR-2-5-3-PARA-2-6-1**](docs/deploy/ATUALIZAR-2-5-3-PARA-2-6-1.md) — Geração de Imagem, `Cloudflare__EncryptionKey`, scripts SQL, modelo de imagem e patch **2.6.2** (criação de contas). Referência intermédia **v2.5.2 → v2.6.0**: [**ATUALIZAR-2-5-2-PARA-2-6-0**](docs/deploy/ATUALIZAR-2-5-2-PARA-2-6-0.md).
