...
**Documentação:** Para **atualizar** o projeto após um pull (desenvolvimento local ou produção Docker), ver **[ATUALIZAR-SERVIDOR-DOCKER-CADDY.md](ATUALIZAR-SERVIDOR-DOCKER-CADDY.md)**. Para **instalação inicial** em servidor com Docker e Caddy, ver **[DEPLOY-DOCKER-CADDY.md](DEPLOY-DOCKER-CADDY.md)**. Em deploy Docker, a base de dados fica no servidor (pasta `data/` exposta ao contentor via volume); scripts manuais podem ser executados no host (ex.: `sqlite3 data/blog.db < backend/api/Migrations/Scripts/nome.sql`) — ver **[EXPOR-DB-NO-HOST.md](EXPOR-DB-NO-HOST.md)**. Os releases são versionados por tag (ex.: `v1.3`, `v1.4`, `v1.5`, `v1.6`); o changelog está em **[CHANGELOG.md](CHANGELOG.md)**. Sitemap e robots.txt são servidos pelo BFF; em deploy com Caddy, configurar o encaminhamento de /sitemap.xml e /robots.txt para o BFF (ver DEPLOY-DOCKER-CADDY). A documentação no repositório é **genérica**; podes manter guias locais com os teus dados em `*-local.md` ou `docs/local/`, que não são commitados.
...
- **Edição de posts** (`/area-autor/posts/novo`, `/area-autor/posts/:id/editar`): título, slug, resumo, conteúdo em **Markdown**, capa, **Publicado** e ordem. Orientação no formulário: proporção recomendada **16:9** (ex.: 1200×675 px) para a imagem de capa. Na página do artigo, capas fora de 16:9 são exibidas inteiras (object-contain, sem corte). Ao criar **novo post**, o campo **Ordem** é pré-preenchido com a próxima posição sugerida (último publicado + 1), editável pelo utilizador. Conteúdo armazenado em Markdown e convertido para HTML na leitura pública.
