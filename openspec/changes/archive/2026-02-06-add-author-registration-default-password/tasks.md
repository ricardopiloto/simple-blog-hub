...
- [x] 2.1 Na página de gestão de contas (AreaContas), no diálogo/modal de **nova conta** (cadastro de autor): remover o campo de senha do formulário. Ao enviar, usar senha padrão `senha123` no payload (ex.: `createUser({ email, author_name, password: 'senha123' })`) ou enviar sem senha se a API aceitar e usar padrão.
