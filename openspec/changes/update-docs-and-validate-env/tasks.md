...
- [ ] 3.1 Criar `.env.example` na raiz com apenas as variáveis utilizadas pelo projeto (ex.: `VITE_BFF_URL=http://localhost:5000`) e um comentário indicando que variáveis da API (ex.: `Admin__Email`) devem ser configuradas no ambiente ao rodar a API, não neste arquivo. Garantir que `.env.example` seja versionado (não está no .gitignore).
