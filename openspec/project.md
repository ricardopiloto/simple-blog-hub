...
- **Páginas**: Início (destaque + recentes), ordenado por data/hora de **criação** — bloco em destaque com etiqueta **"Novo"** = último post criado (entre publicados), Artigos recentes = mesma ordem decrescente; **lista de posts** (`/posts`: paginação, pesquisa com **auto-complete** por autor e título, **filtro por data** com calendário — data única ou intervalo; parâmetros opcionais fromDate/toDate na API/BFF); **post único por slug** (conteúdo do artigo e, no final da página, a descrição do autor quando configurada em Contas), **índice por ordem de história** (`/indice`: paginação de 6 itens com **salto direto para página** no contador (Enter ou blur), filtro em tempo real por número da ordem ou título, cards com título e imagem sem resumo; apenas usuários autenticados podem editar a ordem por **número** ou **arrastar** os cards e persistir); login (`/login`); área do autor (`/area-autor`: página única com "Visão geral do blog" — dashboard com seis indicadores, cards Total/Publicados/Planejados/Rascunho clicáveis para filtrar a lista (borda amarela no ativo; desmarcar ao alterar pesquisa, linha da história ou ordenação), card Autores → Contas — e secção "Publicações" com lista, **pesquisa com auto-complete**, **filtro por data** (calendário, client-side), filtro por linha da história, ordenação configurável, Novo post, editar/excluir; novo/editar post com editor Markdown em rotas separadas; em Editar post é exibido preview da imagem de capa quando há URL).
